...
func TestBasicFail(t *testing.T) {
	...
	vs := viewservice.StartServer(vshost, vsterm)
	...
	s1term := make(chan interface{})
	s1 := StartServer(vshost, port(tag, 1), s1term)
	...
	s2term := make(chan interface{})
	s2 := StartServer(vshost, port(tag, 2), s2term)	
	...
	s3term := make(chan interface{})
	s3 := StartServer(vshost, port(tag, 3), s3term)
	...
	// Can only kill each server once
	//s1.kill()
	//s2.kill()
	s3.kill(s3term)
	time.Sleep(time.Second)
...
func TestPartition1(t *testing.T) {
	...
	s1term := make(chan interface{})
	s1 := StartServer(vshosta, port(tag, 1), s1term)
	delay := int32(0)
...
func TestPartition2(t *testing.T) {
	...
	case <-time.After(10 * time.Second):
		//XXX For W25 only: allow head-of-line blocking between putappend and get
		//t.Fatalf("Get to old primary did not complete")
	}
...
func TestConcurrentSame(t *testing.T) {
	...
	const nservers = 2
	var st [nservers]chan interface{}
	var sa [nservers]*PBServer
	
	for i := 0; i < nservers; i++ {
		st[i] = make(chan interface{})
		sa[i] = StartServer(vshost, port(tag, i+1), st[i])
	}
...
func TestConcurrentSameUnreliable(t *testing.T) {
	...
	fmt.Printf("Test: Concurrent Append()s to the same key; unreliable ...\n")
...
func TestRepeatedCrash(t *testing.T) {
	...
	go func() {
		...
		for atomic.LoadInt32(&done) == 0 {
			...
			sst := make(chan interface{})
			sss := StartServer(vshost, port(tag, i+1), sst)
			samu.Lock()
			st[i] = sst
			sa[i] = sss
			samu.Unlock()
...
func TestRepeatedCrashUnreliable(t *testing.T) {
	...
	ff := func(i int, ch chan int) {
		...
		for atomic.LoadInt32(&done) == 0 {
			...
			val := ck.Get("0")
			if !strings.HasPrefix(val, old_val) {
				t.Fatalf("previous value %v is not a prefix of latest value %v", old_val, val)
			}
			old_val = val
		}
		ret = n
